...
{
        if (!blocked) {
                gpio_direction_output(PRIMOU_GPIO_BT_RESET_N, 1);
                gpio_direction_output(PRIMOU_GPIO_BT_SHUTDOWN_N, 1);
        } else {
                gpio_direction_output(PRIMOU_GPIO_BT_SHUTDOWN_N, 0);
                gpio_direction_output(PRIMOU_GPIO_BT_RESET_N, 0);
        }
        return 0;
}
#endif
...
#if 0
        rc = gpio_request(PRIMOU_GPIO_BT_RESET_N, "bt_reset");
        if (rc)
                goto err_gpio_reset;
        rc = gpio_request(PRIMOU_GPIO_BT_SHUTDOWN_N, "bt_shutdown");
        if (rc)
...
err_rfkill_reg:
        rfkill_destroy(bt_rfk);
err_rfkill_alloc:
        gpio_free(PRIMOU_GPIO_BT_SHUTDOWN_N);
err_gpio_shutdown:
        gpio_free(PRIMOU_GPIO_BT_RESET_N);
err_gpio_reset:
        return rc;
#endif
}
